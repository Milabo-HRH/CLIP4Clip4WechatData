...
def align_state_dicts(chinese_clip_state_dict):
    aligned_state_dict = {}
    
    # 在这里，您可以添加键名转换的规则
    key_mapping = {
        # 根据您提供的键名，添加Chinese-CLIP模型和CLIP模型之间的映射关系
        "bert.embeddings": "text_embedding",
        "bert.encoder.layer": "transformer.layers"
    }

    for k_chinese_clip, v_chinese_clip in chinese_clip_state_dict.items():
        k_clip = k_chinese_clip

        for chinese_clip_key, clip_key in key_mapping.items():
            k_clip = k_clip.replace(chinese_clip_key, clip_key)

        # if k_clip in clip_state_dict_norm:
        #     aligned_state_dict[k_clip] = v_chinese_clip
        #else:
        
        #    print(f"Warning: Key {k_clip} not found in CLIP state_dict.")
